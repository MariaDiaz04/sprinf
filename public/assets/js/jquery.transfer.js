...
(function ($) {
      ...
      if ($(this).is(":checked")) {
        ...
        for (var i = 0; i < groups.length; i++) {
          if (
            ...
            groups.eq(i).parent("div").parent("li").css("display") != "none"
          ) {
        ...
        for (var j = 0; j < groups.length; j++) {
          if (
            ...
            groups.eq(j).parent("div").parent("li").css("display") != "none"
          ) {
  ...
  Transfer.prototype.left_group_items_search_handler = function () {
    ...
    $(self.groupItemSearcherId).on("keyup", function () {
      ...
      //   console.log($(self.groupItemSearcherId).val());
      if ($(self.groupItemSearcherId).val() == "") {
        for (var i = 0; i < transferDoubleGroupListLiUlLis.length; i++) {
          if (
            !transferDoubleGroupListLiUlLis.eq(i).hasClass("selected-hidden")
          ) {
      ...
      let busqueda = $(self.groupItemSearcherId).val().toLowerCase();
      ...
      //   buscar por grupo
      let groupValues = self.$element.find(self.transferDoubleGroupListLiClass);
      ...
      for (var j = 0; j < groupValues.length; j++) {
        ...
        if (!groupValues.eq(j).hasClass("selected-hidden") && display) {
          visiblegroups = true;
          groupValues.eq(j).parent("ul").parent("li").css("display", "block");
          groupValues.eq(j).css("display", "block");
        }
      }

      if (!visiblegroups) {
        for (var j = 0; j < transferDoubleGroupListLiUlLis.length; j++) {
          // console.log(transferDoubleGroupListLiUlLis);
          // PERSONAL CODE - MEJORAR LA BUSQUEDA

          let value = transferDoubleGroupListLiUlLis
            .eq(j)
            .text()
            .trim()
            .toLowerCase();

          let display = value.indexOf(busqueda) > -1;

          if (
            !transferDoubleGroupListLiUlLis.eq(j).hasClass("selected-hidden") &&
            display
          ) {
            transferDoubleGroupListLiUlLis
              .eq(j)
    ...
    $(self.itemSearcherId).on("keyup", function () {
      ...
      if (self.settings.wildcardSearch) {
        for (var j = 0; j < transferDoubleListLis.length; j++) {
          if (
            !transferDoubleListLis.eq(j).hasClass("selected-hidden") &&
            transferDoubleListLis
              .eq(j)
              .text()
              .trim()
              .toLowerCase()
              .indexOf($(self.itemSearcherId).val().toLowerCase()) > -1
          ) {
        ...
        for (var j = 0; j < transferDoubleListLis.length; j++) {
          if (
            ...
            transferDoubleListLis
              ...
              .toLowerCase() == $(self.itemSearcherId).val().toLowerCase()
          ) {
            transferDoubleListLis.eq(j).css("display", "block");
          }
        }
    ...
    for (var i = 0; i < checkboxSelectedItems.length; ) {
      ...
      if (
        ...
        this.$element
          ...
          .removeAttr("disabled");
        this.$element
          .find("#group_" + groupIndex + "_checkbox_" + index + "_" + this.id)
          .prop("checked", false)
          .parent("div")
          .parent("li")
          .css("display", "")
  ...
  Transfer.prototype.move_selected_items = function () {
    ...
    for (
      ...
      if (
        checkboxSelectedItems.eq(i).parent("div").parent("li").css("display") !=
          "none" &&
        checkboxSelectedItems.eq(i).is(":checked")
      ) {
        ...
        for (var i = 0; i < transferDoubleSelectedListLis.length; i++) {
          if (
            transferDoubleSelectedListLis
              ...
              .trim()
              .toLowerCase()
              .indexOf($(self.selectedItemSearcherId).val().toLowerCase()) > -1
          ) {
        ...
        for (var i = 0; i < transferDoubleSelectedListLis.length; i++) {
          if (
            transferDoubleSelectedListLis
              ...
              .text()
              .trim()
              .substr(0, $(self.selectedItemSearcherId).val().length)
              .toLowerCase() ==

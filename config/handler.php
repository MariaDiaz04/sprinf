    ...
    function __construct($route) {
        ...
        if (isset($route['type'])) {
           // return var_dump($class->{$route['method']}); 
            switch ($route['type']) {
                case 'GET':  $class->{$route['method']}(array_diff($_GET, array('r'))); break;
                case 'POST': $class->{$route['method']}($_POST); break;
                case 'FILES': $class->{$route['method']}($_FILES); break;
                case 'REQUEST': $class->{$route['method']}($_REQUEST); break;
                default:
                require '../app/controllers/controller.php';

...
class respaldoController extends Controller
{
  ...
  {
    return $this->view('respaldos/respaldar');
  }
  public function respaldar()
  {
    ...
    $dir = ROOT . "/public/assets/respaldo/";
    $day = date("d");
    $mont = date("m");
    $year = date("Y");
    $hora = date("H-i-s");
    ...
    $result = mysqli_query($con, 'SET CHARACTER SET utf8;', 1);
    ...
    if ($result) {
      ...
      foreach ($tables as $table) {
        if (str_contains($table, 'detalles_') || str_contains($table, 'estudiantes_pendientes_a_proyecto')) {
          $sql .= "\n";
        } else {
  ...
  {
    $usuario = new Usuario;
    $response = $usuario->checkUser($this->limpiaCadena($_POST['password']));
    if ($response) {
